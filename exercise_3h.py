...
import phases
import covid_state_v2 as state
import events


def run_simulation(args):
    # Setup the description of the disease.
    state.read_from_file(args.states)
...
def plot_graphs(sim_state):
    s.graph_simulation(
        sim_state, f'Cumulative Count Simulation, {sim_state[s.POPULATION]} population',
    ...
    s.graph_simulation(
        sim_state, f'Active Count Simulation, {sim_state[s.POPULATION]} population',
        [s.ACTIVE_CASES_SERIES, s.ACTIVE_CONFIRMED_CASES_SERIES,
         s.ACTIVE_HOSPITALIZED_CASES_SERIES, s.ACTIVE_ICU_CASES_SERIES])

    s.graph_simulation(
        sim_state, f'New Daily Count Simulation, {sim_state[s.POPULATION]} population',
